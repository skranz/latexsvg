example.latexsvg = function() {
  ...
  res = latexsvg(file="latex.svg", do.stop = FALSE)
...
#' @export
latexsvg = function(svg=paste0(readLines(file),collapse="\n"), file=NULL, outfile=if (!is.null(file)) paste0(tools::file_path_sans_ext(file),"_latex.svg"), do.stop = TRUE) {
  ...
  app$ui = bootstrapPage(
    tags$script(type="text/javascript",
      ...
      src="latexsvg/MathJax-2.2/MathJax.js?config=default"
    ),
...
$(document).on("EndSVGLatex",function(e) {
  ...
  save.svg.handler =  function(...) {
    ...
    svg = args$svg
    svg = gsub('xlink:href="#','href="#', svg,fixed = TRUE)
